 ...
 * 1.0.0.0
 */
(function(window,$,saas,nova){"use strict";
	...
	var monthSize={0:31,1:28,2:31,3:30,4:31,5:30,6:31,7:31,8:30,9:31,10:30,11:31};
	...
	var DATE_FORMATS={yyyy:["FullYear",4],
		yy:["FullYear",2,0],
		MM:["Month",2,1],
		M:["Month",1,1],
		dd:["Date",2],
	...
	Factory.defaults={
		...
		englishWeekTitle:{0:'sun',1:'mon',2:'tue',3:'wed',4:'thu',5:'fri',6:'sat'},
		weekShortTitle:{0:"日",1:"一",2:"二",3:"三",4:"四",5:"五",6:"六"},
		wrapClass:"",
		weekTitle:{0:"星期日",1:"星期一",2:"星期二",3:"星期三",4:"星期四",5:"星期五",6:"星期六"},
		...
		festival:{
			'2016-01-01':{name:'元旦',vacationName:'元旦'},
			'2016-02-07':{name:'除夕',vacationName:'除夕'},
			'2016-02-08':{name:'春节',vacationName:'春节'},
			'2016-02-09':{vacationName:'春节假期'},
			'2016-02-10':{vacationName:'春节假期'},
			'2016-02-11':{vacationName:'春节假期'},
			'2016-02-12':{vacationName:'春节假期'},
			'2016-02-13':{vacationName:'春节假期'},
			'2016-04-02':{vacationName:'清明节假期'},
			'2016-04-03':{vacationName:'清明节假期'},
			'2016-04-04':{name:'清明',vacationName:'清明节'},
			'2016-04-30':{vacationName:'劳动节假期'},
			'2016-05-01':{name:'劳动',vacationName:'劳动节'},
			'2016-05-02':{vacationName:'劳动节假期'},
			'2016-06-09':{name:'端午',vacationName:'端午节'},
			'2016-06-10':{vacationName:'端午节假期'},
			'2016-06-11':{vacationName:'端午节假期'},
			'2016-09-15':{name:'中秋',vacationName:'中秋节假期'},
			'2016-09-16':{vacationName:'中秋节假期'},
			'2016-09-17':{vacationName:'中秋节'},
			'2016-10-01':{name:'国庆',vacationName:'国庆节'},
			'2016-10-02':{vacationName:'国庆节假期'},
			'2016-10-03':{vacationName:'国庆节假期'},
			'2016-10-04':{vacationName:'国庆节假期'},
			'2016-10-05':{vacationName:'国庆节假期'},
			'2016-10-06':{vacationName:'国庆节假期'},
			'2016-10-07':{vacationName:'国庆节假期'},
			'2016-12-31':{vacationName:'元旦假期'},
			'2017-01-01':{name:'元旦',vacationName:'元旦'},
			'2017-01-02':{vacationName:'元旦假期'},
			'2017-01-27':{name:'除夕',vacationName:'除夕'},
			'2017-01-28':{name:'春节',vacationName:'春节'},
			'2017-01-29':{vacationName:'春节假期'},
			'2017-01-30':{vacationName:'春节假期'},
			'2017-01-31':{vacationName:'春节假期'},
			'2017-02-01':{vacationName:'春节假期'},
			'2017-02-02':{vacationName:'春节假期'},
			'2017-04-02':{vacationName:'清明节假期'},
			'2017-04-03':{vacationName:'清明节假期'},
			'2017-04-04':{name:'清明',vacationName:'清明节'},
			'2017-04-29':{vacationName:'劳动节假期'},
			'2017-04-30':{vacationName:'劳动节假期'},
			'2017-05-01':{name:'劳动',vacationName:'劳动节'},
			'2017-05-28':{vacationName:'端午节假期'},
			'2017-05-29':{vacationName:'端午节假期'},
			'2017-05-30':{name:'端午',vacationName:'端午节'},
			'2017-10-01':{name:'国庆',vacationName:'国庆节'},
			'2017-10-02':{vacationName:'国庆节、中秋节假期'},
			'2017-10-03':{vacationName:'国庆节、中秋节假期'},
			'2017-10-04':{name:'中秋',vacationName:'中秋节'},
			'2017-10-05':{vacationName:'国庆节、中秋节假期'},
			'2017-10-06':{vacationName:'国庆节、中秋节假期'},
			'2017-10-07':{vacationName:'国庆节、中秋节假期'},
			'2017-10-08':{vacationName:'国庆节、中秋节假期'}},
		...
		Calendar.prototype={
			...
			init:function(options){
				...
				if(typeof options.template=="object"){
					...
					switch(options.template){
						case"big":template=bigTemplate;
						break;
						case"birthday":template=birthdayTemplate;
						break;
						case"small":template=smallTemplate;
						break;
						default:template=smallTemplate;
						break;
					}
				}
			...
			triggerEventHandler:function(e){
				...
				if(self.options.cascading){
					...
					if(!start){
						if(triggerFirstValue){
							start=triggerFirstValue;self.cascadingSelected.start=start
						}
					}
					if(!end){
						if(triggerLastValue){
							end=triggerLastValue;self.cascadingSelected.end=end
						}
					}
				...
				getOffset:function(){
					...
					var left=this.getInt($trigger.offset().left+offsetAmount.left);return{top:top,left:left};
				},
...
i+'</li>');
				...
				$ul.html(liStr);$hourDropdown.append($ul);
			...
			birthdayOkHandler:function(e){
				e.stopPropagation();
				var self=e.data.self;self.inputTime();
				self.destroy();
		...
		inputTime:function(){
			...
			var start=self.cascadingSelected.start;
			var end=self.cascadingSelected.end;
			var result;if(self.options.cascading){
		...
		yearDropdownHandler:function(e){
			...
			var offset={top:$this.offset().top,left:$this.offset().left};
			var calDate=self.now;var calYear=calDate.getFullYear();
			var $yearCurrentLi=$yearDropdown.find("[data-year="+calYear+"]");
			$yearDropdown.css({top:offset.top-calOffset.top,left:offset.left-calOffset.left});
			...
			var currentLiTop=currentLiHeight*currentLiIndex;$yearDropdown.find(".cal-year-dropdown").scrollTop(currentLiTop)
		},
		...
		createMonthDropdown:function(){
			var minLimitDate=this.minLimitDate;
			var maxLimitDate=this.maxLimitDate;
			var minYear=minLimitDate?minLimitDate.getFullYear():1900;var maxYear=maxLimitDate?maxLimitDate.getFullYear():2099;
			...
			var $dropdownBox=$(''+'<div class="cal-month-dropdown-box">'+'    <div class="cal-month-select-active">'+(this.now.getMonth()+1)+'月<i></i></div>'+'</div>');
			var $ul=$('<ul class="cal-month-dropdown"></ul>');
			$dropdownBox.append($ul);var liStr=[];
		...
		createYearDropdown:function(){
			...
			var maxLimitDate=this.maxLimitDate;
			var minYear=minLimitDate?minLimitDate.getFullYear():1900;var maxYear=maxLimitDate?maxLimitDate.getFullYear():2099;
			var $dropdownBox=$(''+'<div class="cal-year-dropdown-box">'+'    <div class="cal-year-select-active">'+this.now.getFullYear()+'年<i></i></div>'+'</div>');
		...
		bindSelectHandler:function(e){
			...
			if($this.children().first().is(".nodate,.caldisabled")){
				...
				if(dateStr){
					...
					if(!self.options.sectionSelect){self.now=new Date(date);
					}
				}
				var end=self.cascadingSelected.end;
				if(end){
				...
				if(self.options.cascading){
					...
					if(self.options.allowMutiSelected){
						...
						if(isSelect){
							...
							if(autoRender){runSelectDateCallback();
							}
						}
					}else{
						self.selected={};
		...
		bindMonthChangeEvent:function(){
			...
			$monthPrev.on("click",{
				self:this,monthOffset:-monthChangeOffset
		...
		dateValidate:function(){
			...
			if(end){
				...
				var endTime=this.cascadingSelected.endTime;endDate.setHours(endTime.getHours());
		...
		DateLimit:function(){
			if(this.maxLimitDate){
				...
				if(isSameMonthNext){this.hideMonthNext();
				}
			}
		...
		renderTime:function(){
			if(this.options.hasTime){
				this.wrap.find(".cal-pane").children().css({"display":"inline-block"});
			}
			if(this.options.cascading){
				...
				if(this.cascadingIndex===0){
					...
					if(start){
						...
						if(end&&$endDate.length>0){
							...
							var size={width:$endDate.width(),height:$endDate.height(),
							...
							$numberOfDays.css({left:0,top:0});
							var $numberOfDaysSpan=$numberOfDays.children("span");
							var template=self.options.numberOfDays;
							var numberOfDaysHtml=self.replaceWith(template,{num:numberOfDays});
					...
					if(end){
						if(!mouseOverEnd){$endDate.addClass("td-selected");
					}
				}
			}
		...
		renderFestival:function(){
			...
			$tds.each(function(i,domEle){
				...
				if(self.options.division&&thisDateDay==1){
					...
					}else{$calday.html((thisDateMonth+1)+"月"+thisDateDay+"日");
				}
			}
		}
		)},
		...
		createBody:function(){
			...
			html=this.replaceWith(options.template.calWrap,{content:html});
			return html;
		},
		createSingleCalendar:function(date,offset){
			...
			html+=this.replaceWith(options.template.calTitle,{
				month:this.replaceWith(options.titleTip,{year:year,month:month+1})
			});
			if(this.options.division){
				var nextMonthTitle=this.replaceWith(options.template.calTitle,{month:this.replaceWith(options.titleTip,{year:nextMonthYear,month:nextMonthMonth+1})});
				    nextMonthTitle=nextMonthTitle.replace("caltitle","caltitle caltitlenext");html+=nextMonthTitle}
				...
				var weekShortTitle=this.options.weekShortTitle;html+=this.replaceWith(options.template.calBody,{month:month+1,
				...
				html=this.replaceWith(options.template.calMonth,{content:html,offset:'data-offset="'+offset+'"'});
				return html;
			},
				...
				dates.forEach(function(row){
					...
					row.forEach(function(cal){
						if(cal){
							...
							var singleDateHtml=self.replaceWith(options.template.day,{week:cal.getDay(),day:Calendar.dateFormat(cal,"dd"),className:self.getClass(cal,date),dateMap:'data-date-map="'+dateMap+'"'});
						}else{
							singleDateHtml='<td><div class="date caldate caldisabled"></div></td>';
						}
						dateHtml+=singleDateHtml;
					});
					html+=self.replaceWith(options.template.weekWrap,{week:dateHtml});
				});
				return html;
			},
				...
				for(row=0;row<6;row++){
					...
					for(col=0;col<7;col++){
						...
						if(space&&(spaceStart||(+nextMonthFirstDate==+newDate))){
							space--;dateArray[row][col]=null;
						}else{
							index++;dateArray[row][col]=newDate;
						}
					}
				}
			}else{
				for(row=0;row<6;row++){dateArray[row]=[];
					for(col=0;col<7;col++){
						index++;
		...
		getSelect:function(){
			...
			if(cascading){
				...
				var selected=[];
				for(var s in this.selected){selected.push(s)
			}
			return selected;
		}},
		getDayOfWeek:function(date){
			date=date||this.now;if(date){
